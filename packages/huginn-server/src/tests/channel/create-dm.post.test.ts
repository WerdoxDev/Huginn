...
import { type APIPostDMChannelResult, ChannelType } from "@huginnjs/shared";
...
describe("POST /users/@me/channels", () => {
   test("should return 'Invalid Form Body' when body constrains are not met", async () => {
      ...
      const result2 = testHandler("/api/users/@me/channels", authHeader(user.accessToken), "POST", {
         recipients: [],
      }).then(removeChannelLater);
      expect(result2).rejects.toThrow("Invalid Form Body"); // Invalid id

      const result3 = testHandler("/api/users/@me/channels", authHeader(user.accessToken), "POST", {
         recipients: ["000000000000000000"],
      }).then(removeChannelLater);
   ...
   test("should return 'Unauthorized' when no token is passed", async () => {
      ...
      const result = testHandler("/api/users/@me/channels", {}, "POST", {
