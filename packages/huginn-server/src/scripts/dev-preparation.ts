...
for (const user of internalUsers) {
   ...
   createdInternalUsers.push(await prisma.user.create({
      data: {
         id: snowflake.generate(WorkerID.TESTING),
         email: `${user}@gmail.com`,
         flags: UserFlags.NONE,
...
await prisma.channel.create({ data: { id: snowflake.generate(WorkerID.TESTING), type: ChannelType.DM, recipients: { connect: createdInternalUsers.map(x => ({ id: x.id })) } } })
