...
export type JsonCode = (typeof JsonCode)[keyof typeof JsonCode];
...
export type GatewayCode = (typeof GatewayCode)[keyof typeof GatewayCode];

export const FieldCode = {
   REQUIRED: "REQUIRED",
   SOFT_REQUIRED: "SOFT_REQUIRED",
   INVALID_LOGIN: "INVALID_LOGIN",
   WRONG_LENGTH: "INCORRECT_LENGTH",
...
export const VoiceSignallingError = {
   UNKNOWN_ERROR: 0,
   UNKNOWN_PRODUCER: 1001,
   UNKNOWN_CONSUMER: 1002,
   UNKNOWN_TRANSPORT: 1003,
   UNKNOWN_MEDIA_KIND: 1004,
   WRONG_TRANSPORT_DIRECTION: 2001,
   ROUTER_CANT_CONSUME: 2002,
} as const;

export type VoiceSignallingError = (typeof VoiceSignallingError)[keyof typeof VoiceSignallingError];

export const OAuthCode = {
   CANCELLED: "CANCELLED",
} as const;

export type OAuthCode = (typeof OAuthCode)[keyof typeof OAuthCode];
...
export const Fields = {
   ...
   wrongLength(min?: number, max?: number): [string, string] {
      ...
      else if (min) text = text = `This must be at least ${min} characters long`;
      else if (max) text = `This must be at most ${max} characters long`;
      else text = "This is invalid";
      return [text, FieldCode.WRONG_LENGTH];
   },
   ...
   usernameInvalid(): [string, string] {
      return ["Username can only include numbers, letters, underlines _ and fullstops .", FieldCode.USERNAME_INVALID];
   },
