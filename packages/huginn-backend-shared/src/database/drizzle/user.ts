...
import { selectPrivateUser, type UserArgs, type UserPayload } from "./common";
import { drizzle } from "./db";

export const userRepo = {
   async getById<Args extends UserArgs>(id: Snowflake, args?: Args) {
      return analytics.startActiveSpan("db.d.user.getById", async (span) => {
         span.setAttribute("query.user.id", id);
         assertId("d.user.getById", id);

         try {
            const user = await drizzle.user.findUnique({ where: { id: BigInt(id) }, ...args }).throw();
            return idFix(user as UserPayload<Args>);
         } catch (e) {
            recordSpanError(e);
            await assertExists(e, "d.user.getById", DBErrorType.NULL_USER, [id]);
            throw e;
         }
      });
   },
   async getByUsername<Args extends UserArgs>(username: string, args?: Args) {
      return analytics.startActiveSpan("db.d.user.getByUsername", async (span) => {
         span.setAttribute("query.username.length", username.length);

         try {
            const user = await drizzle.user.findUnique({ where: { username: username }, ...args }).throw();
            return user as UserPayload<Args>;
         } catch (e) {
            recordSpanError(e);
            await assertExists(e, "d.user.getByUsername", DBErrorType.NULL_USER, [username]);
            throw e;
         }
      });
   },
   async findByCredentials(options: APIPostLoginJSONBody) {
      return analytics.startActiveSpan("db.d.user.findByCredentials", async (span) => {
         span.setAttributes({
         ...
         try {
            const user = await drizzle.user.findFirst({
               where: {
            ...
            assertObj("user.findByCredentials", user, DBErrorType.NULL_USER);
            ...
            assertCondition("user.findByCredentials", !passwordValid, DBErrorType.NULL_USER);

            return idFix(omit(user, ["password"]));
         } catch (e) {
            recordSpanError(e);
            throw e;
         }
   ...
   async edit<Args extends UserArgs>(id: Snowflake, editedUser: APIPatchCurrentUserJSONBody, args?: Args) {
      return analytics.startActiveSpan("db.d.user.edit", async (span) => {
         span.setAttributes({
            "query.user.id": id,
         ...
         try {
            ...
            const updatedUser = await drizzle.user
               .update({
                  where: { id: BigInt(id) },
                  data: {
   ...
   async createOne(options: { id?: bigint; username: string; displayName?: string | null; password?: string | null; email: string; avatar?: string | null }) {
      return analytics.startActiveSpan("db.d.user.createOne", async (span) => {
         span.setAttributes({
         ...
         try {
            ...
            const newUser = await drizzle.user.create({
               data: {
   ...
   async hasChannel(userId: Snowflake, channelId: Snowflake) {
      return analytics.startActiveSpan("db.user.hasChannel", async (span) => {
         ...
         try {
            const hasAccess = await drizzle.user.exists({
               where: {
                  id: BigInt(userId),
                  OR: [{ includedChannels: { some: { channel: { is: { id: BigInt(channelId) } } } } }, { ownedChannels: { some: { id: BigInt(channelId) } } }],
               },
            });
