	...
	return (
		<div className={clsx("w-52 rounded-lg bg-secondary", props.className)}>
			<Listbox value={dropdownContext.selected} onChange={dropdownContext.onChange}>
				{({ open, value }) => (
					<>
						<ListboxButton className="relative flex w-full select-none items-center justify-between overflow-hidden p-2 text-white outline-none">
...
function ItemsWrapper(props: { className?: string; children?: ReactNode }) {
	return (
		<ListboxOptions
			...
			transition
			className={clsx(
				"flex cursor-pointer flex-col gap-y-0.5 rounded-lg bg-secondary p-1 outline-none transition [--anchor-gap:0.25rem] [--anchor-padding:1rem] data-[closed]:translate-y-5 data-[closed]:opacity-0",
				props.className,
			)}
		>
			{props.children}
