...
export default function DirectMessageChannel(props: {
  ...
  const { tryMutate } = useSafeDeleteDMChannel(
    props.channel.id,
    props.channel.type,
    props.channel.name,
  );
  ...
  const isLoading =
    state.isLoading &&
    state.location.pathname === `/channels/@me/${props.channel.id}`;
  ...
  return (
    <li
      ...
      className={clsx(
        "group relative flex shrink-0 cursor-pointer overflow-hidden rounded-md",
      )}
      data-context={
        context?.isOpen && data?.id === props.channel.id ? true : undefined
      }
    >
      <Link
        preload="intent"
      ...
      >
        {props.channel.type === ChannelType.DM ? (
          <UserAvatar
            ...
            avatarHash={recipients[0]?.avatar}
            className="mr-3"
            animatedMode="hover"
            hovered={isHovered}
          />
        ...
        {!isLoading ? (
          <div className="mr-2 flex shrink-0 items-center gap-x-2">
            {props.pinned && (
              <IconMingcutePinFill className="text-text/50 size-4 transition-opacity" />
            )}
            <button
              className="group/close ml-auto hidden cursor-pointer group-hover:block"
              onClick={handleLeave}
            >
