...
export default function MockDefaultMessage(props: {
   ...
   return (
      <div
         className={clsx(
            "hover:bg-surface-alt flex flex-col items-start gap-y-2 p-2",
      ...
      >
         ...
         <div className="overflow-hidden font-light text-white">
            <div
               className={clsx(
                  "px-2.5 py-1.5 font-normal wrap-anywhere text-white",
                  props.separate && "rounded-t-xl",
                  props.self ? "bg-primary-800" : "bg-surface",
                  props.end && "rounded-b-xl!",
               )}
               style={{
                  borderTopRightRadius: props.roundedTop,
                  borderBottomRightRadius: props.roundedBottom,
               }}
            >
               {props.text}
            </div>
