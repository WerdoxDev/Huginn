...
export default function SettingsVoiceTab(props: SettingsTabProps) {
	...
	const inputDevices = useMemo(
		() => data?.filter((x) => x.kind === "audioinput"),
		[data],
	);
	const outputDevices = useMemo(
		() => data?.filter((x) => x.kind === "audiooutput"),
		[data],
	);
	const cameraDevices = useMemo(
		() => data?.filter((x) => x.kind === "videoinput"),
		[data],
	);
	...
	useEffect(() => {
		...
		setSelectedInput(
			inputDevices?.find((x) => x.deviceId === settings.local.inputDeviceId) ??
				inputDevices[0],
		);
		setSelectedOutput(
			outputDevices?.find(
				(x) => x.deviceId === settings.local.outputDeviceId,
			) ?? outputDevices[0],
		);
		setSelectedCamera(
			cameraDevices?.find(
				(x) => x.deviceId === settings.local.cameraDeviceId,
			) ?? cameraDevices[0],
		);
	}, [data]);
				...
				<HuginnTab.TabPanels className="mt-5">
					<HuginnTab.TabPanel>
						<div className="flex gap-x-5">
							<HuginnDropdown
								...
								defaultValue={inputDeviceOptions.find(
									(x) => x.value === selectedInput?.deviceId,
								)}
							>
							...
							<HuginnDropdown
								...
								defaultValue={outputDeviceOptions.find(
									(x) => x.value === selectedOutput?.deviceId,
								)}
							>
						...
						<div className="mt-5 flex gap-x-5">
							<div className="w-full max-w-xs">
								...
								<RangeInput
									...
									defaultValue={settings.local.inputVolume}
								/>
							</div>
							<div className="w-full max-w-xs">
								...
								<RangeInput
									...
									defaultValue={settings.local.outputVolume}
									maxValue={200}
								/>
							</div>
						</div>
						<div className="mt-5 flex">
							<div className="w-full max-w-165">
								...
								<RangeInput
									...
									defaultValue={remap(
										settings.local.inputThreshold ?? -100,
										-100,
										0,
										0,
										100,
									)}
									getTooltipText={(percentage) =>
										`${remap(percentage, 0, 100, -100, 0)}db`
									}
								>
									<div
					...
					<HuginnTab.TabPanel className="flex flex-col gap-y-5">
						<HuginnDropdown
							...
							defaultValue={cameraDeviceOptions.find(
								(x) => x.value === selectedCamera?.deviceId,
							)}
						>
