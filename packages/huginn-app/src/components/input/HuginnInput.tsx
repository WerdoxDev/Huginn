...
export default function HuginnInput(props: HuginnInputProps) {
  const [id, _setId] = useState(() => Math.random().toString());
  ...
  return (
    ...
    >
      <div
        className={clsx(!props.headless && "flex flex-col", props.className)}
      >
        {props.children}
      </div>
    </InputContext.Provider>
...
function Input(props: { headless?: boolean; className?: string }) {
  ...
  useEffect(() => {
    setTimeout(() => {
      if (
        inputContext.value !== undefined &&
        inputContext.inputRef?.current &&
  ...
  return (
    <input
      ...
      className={clsx(
        !props.headless &&
          "w-full bg-transparent p-2 text-white placeholder-text/60 outline-none disabled:cursor-not-allowed",
        props.className
      )}
...
function Label(props: {
  ...
  return (
    <label
      ...
      className={clsx(
        !props.headless &&
          "select-none font-medium text-xs uppercase opacity-90",
      ...
      {!props.hideAdditional &&
        (inputContext.status.text ? (
          <span
            className={clsx(
              "text-error",
              inputContext.status.text && "font-normal normal-case italic"
            )}
          >
