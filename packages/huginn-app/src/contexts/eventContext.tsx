...
type EventContextType = {
  ...
  listenEvent<K extends keyof EventTypes>(
    type: K,
...
export function EventProvider(props: { children?: ReactNode }) {
  ...
  const dispatchEvent = useCallback<EventContextType["dispatchEvent"]>(
    (type, data) => {
  ...
  function listenEvent<K extends keyof EventTypes>(
    type: K,
    callback: (data: EventTypes[K]) => void
  ) {
    ...
    return () => {
      const newListeners = { ...listeners.current };
      if (newListeners[type]) {
        newListeners[type] = newListeners[type].filter(
  ...
  return (
    <EventContext.Provider value={{ dispatchEvent, listenEvent }}>
      {props.children}
