...
test("voice connection is successful", async ({ page }) => {
   ...
   await page
      ...
      .locator(".ml-auto > button:nth-child(1)")
      .click();
...
test("screensharing works successfully", async ({ headless }) => {
   const browser = await chromium.launch({
      headless: headless,
      args: ["--use-fake-ui-for-media-stream", "--use-fake-device-for-media-stream", "--allow-http-screen-capture"],
   });
...
test("camera works successfully", async ({ headless }) => {
   ...
   await expect(
      page.locator(
         ".group\\/element.relative.flex.shrink-0.flex-col.items-center.justify-center.gap-y-1.shadow-md.transition-shadow.hover\\:shadow-xl.aspect-video.overflow-hidden.p-0.bg-surface > .p-5",
      ),
   ).not.toBeInViewport();
   await expect(page.locator("video")).toBeInViewport();
});
