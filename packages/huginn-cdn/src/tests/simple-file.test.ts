...
for (const category of categories) {
   describe(`${category} operations`, () => {
      test(`should return 'Invalid Form Body' when body constrains are not met`, async () => {
         const result = testHandler(`/cdn/${category}/123`, await cdnTokenHeader(), "POST");
      ...
      test("should return the file name when file upload is successful", async () => {
         ...
         const result = (await testHandler(`/cdn/${category}/123`, await cdnTokenHeader(), "POST", formData, true)) as Response;
         expect(await result.text()).toBe("pixel.png");
      });

      test("should return an exact file when request is successful", async () => {
         const result = (await testHandler(`/cdn/${category}/123/pixel.png`, {}, "GET", undefined, true)) as Response;
      ...
      test(`should return an image in different formats even if they aren't cached / don't exist`, async () => {
         const results = (await Promise.all([
            testHandler(`/cdn/${category}/123/pixel.jpeg`, {}, "GET", undefined, true),
            testHandler(`/cdn/${category}/123/pixel.jpg`, {}, "GET", undefined, true),
            testHandler(`/cdn/${category}/123/pixel.webp`, {}, "GET", undefined, true),
      ...
      test(`should return 'File Not Found' when a file with doesn't exist`, async () => {
         const result = testHandler(`/cdn/${category}/123/invalid.png`, {}, "GET");
      ...
      test(`should return 'Invalid File Format' when the specified file format is invalid`, async () => {
         const result = testHandler(`/cdn/${category}/123/pixel.gif`, {}, "GET");
