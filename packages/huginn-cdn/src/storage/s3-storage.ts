...
export class S3Storage extends Storage {
	...
	public async getFile(category: FileCategory, name: string): Promise<ReadableStream | undefined> {
		try {
			const cmd = new GetObjectCommand({ Bucket: AWS_BUCKET, Key: pathe.join(category, name) });
	...
	public async exists(category: FileCategory, name: string): Promise<boolean> {
		try {
			const cmd = new HeadObjectCommand({ Bucket: AWS_BUCKET, Key: pathe.join(category, name) });
			const result = await this.s3.send(cmd);
			return true;
		} catch (e) {
			logFileNotFound(category, name);
