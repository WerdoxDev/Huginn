...
export default function CustomList({
  ...
  const initialOption = useMemo(() => {
    return (
      options.find((option) => option.text.trim().toLowerCase() === defaultValue) ??
      options[0] ??
      null
    );
  ...
  return (
    ...
    >
      <div className="relative mx-2 select-none">
        <Listbox.Button
          className={`flex items-center rounded-md bg-secondary px-2 py-1 ${className ?? ""}`.trim()}
        >
        ...
        <Listbox.Options className="absolute mt-1 w-full overflow-hidden rounded-md bg-secondary shadow-lg">
          {options.map((option) => (
            <Listbox.Option
              key={option.text}
              value={option}
              ...
              className={({ active, disabled }) =>
                ...
                } ${disabled ? "cursor-not-allowed text-text/50" : ""} ${option.hidden ? "hidden" : ""}`
              }
            >
