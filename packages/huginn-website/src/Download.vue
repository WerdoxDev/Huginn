...
<template>
   <div class="mt-16 flex h-full w-full items-center justify-center">
      <div class="hidden w-full max-w-5xl flex-col md:flex">
         ...
         <div class="text-lg">
            <div class="flex flex-row" v-if="latestInfo && platformKind && versionKind">
               ...
               <CustomList class="w-32" @changed="selected => {
                  versionKind = selected;
               }
                  " :default="versionKind" :options="[
            ...
            <div class="mt-6 flex items-center" v-if="latestInfo && platformKind && versionKind">
               <button @click="download(platformKind!, versionKind!)"
                  class="flex w-max flex-row items-center gap-x-2 rounded-md bg-primary px-4 py-2 transition-all hover:bg-primary/50"
                  :class="{ 'bg-primary/50': versionKind === 'none' }" :disabled="versionKind === 'none'">
                  <Icon icon="mingcute:download-3-fill" />
                  {{ versionKind !== "none" ? "Download Huginn " + buttonVersionText : "Select a Version" }}
               </button>

               <div class="ml-4 flex h-full w-fit items-center rounded-md bg-success/30 px-4 py-2"
                  :class="{ hidden: versionKind === 'none' }">
                  <Icon icon="clarity:date-solid" class="mr-2" />
                  {{ dateText }}
               </div>

               <div class="ml-4 flex h-full w-fit items-center rounded-md bg-[#00A7E0]/30 px-4 py-2"
      ...
      <div class="flex flex-col items-center justify-center gap-12 px-6 md:hidden">
         ...
         <div class="text-center text-2xl">Unfortunately <span class="font-bold">Huginn</span> is not available for your
            device.</div>
         <RouterLink to="/" class="flex items-center text-accent underline">
